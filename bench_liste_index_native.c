...
int main(void){
ClairIntList nombres = clair_int_list_make((long long[]){10LL, 20LL, 30LL, 40LL, 50LL}, 5LL);
long long somme = 0LL;
...
while (((clair_range_index_1 < clair_range_fin_1))) {
    ...
    somme = (somme + clair_int_list_get(&nombres, (long long)(((long long)(i) % (long long)(5LL)))));
    clair_range_index_1 = (clair_range_index_1 + clair_range_pas_1);
}
(void)({ NvCall __c = nv_call_new(); nv_call_add(&__c, nv_int(somme)); NvVal __r = nv_dispatch_call("ecris", __c.args, __c.argc, __c.kw); nv_call_free(&__c); __r; });
    return 0;
}
