...
int main(void){
ClairDoubleList valeurs = clair_double_list_make((double[]){(double)(1LL), 2.5, (double)(3LL)}, 3LL);
clair_double_list_append(&valeurs, (double)(4LL));
(void)({ NvCall __c = nv_call_new(); nv_call_add(&__c, nv_str("Liste :")); nv_call_add(&__c, clair_double_list_box(&valeurs)); NvVal __r = nv_dispatch_call("ecris", __c.args, __c.argc, __c.kw); nv_call_free(&__c); __r; });
(void)({ NvCall __c = nv_call_new(); nv_call_add(&__c, nv_str("Premier :")); nv_call_add(&__c, nv_float(clair_double_list_get(&valeurs, (long long)(0LL)))); NvVal __r = nv_dispatch_call("ecris", __c.args, __c.argc, __c.kw); nv_call_free(&__c); __r; });
(void)({ NvCall __c = nv_call_new(); nv_call_add(&__c, nv_str("Deuxieme :")); nv_call_add(&__c, nv_float(clair_double_list_get(&valeurs, (long long)(1LL)))); NvVal __r = nv_dispatch_call("ecris", __c.args, __c.argc, __c.kw); nv_call_free(&__c); __r; });
(void)({ NvCall __c = nv_call_new(); nv_call_add(&__c, nv_str("Longueur :")); nv_call_add(&__c, nv_int((long long)valeurs.len)); NvVal __r = nv_dispatch_call("ecris", __c.args, __c.argc, __c.kw); nv_call_free(&__c); __r; });
    return 0;
}
