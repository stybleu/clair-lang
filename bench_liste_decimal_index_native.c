...
int main(void){
ClairDoubleList valeurs = clair_double_list_make((double[]){1.5, 2.5, 3.5, 4.5, 5.5}, 5LL);
double somme = 0.0;
...
while (((clair_range_index_1 < clair_range_fin_1))) {
    ...
    somme = (somme + clair_double_list_get(&valeurs, (long long)(((long long)(i) % (long long)(5LL)))));
    clair_range_index_1 = (clair_range_index_1 + clair_range_pas_1);
}
(void)({ NvCall __c = nv_call_new(); nv_call_add(&__c, nv_float(somme)); NvVal __r = nv_dispatch_call("ecris", __c.args, __c.argc, __c.kw); nv_call_free(&__c); __r; });
    return 0;
}
