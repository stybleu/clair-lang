...
int main(void){
    ...
    clair_literal_2 = nv_str("Texte :");
    clair_literal_3 = nv_str("Longueur :");
ClairString base = clair_string_literal("A");
ClairString texte = clair_string_literal("");
long long clair_range_index_1 = 0LL;
long long clair_range_fin_1 = 5000000LL;
long long clair_range_pas_1 = 1LL;
while (((clair_range_index_1 < clair_range_fin_1))) {
    long long i = clair_range_index_1;
    clair_string_assign_concat_many(&texte, (ClairString[]){base, clair_string_literal("lice")}, 2LL);
    clair_range_index_1 = (clair_range_index_1 + clair_range_pas_1);
}
(void)({ NvCall __c = nv_call_new(); nv_call_add(&__c, clair_literal_2); nv_call_add(&__c, clair_string_box(texte)); NvVal __r = nv_dispatch_call("ecris", __c.args, __c.argc, __c.kw); nv_call_free(&__c); __r; });
(void)({ NvCall __c = nv_call_new(); nv_call_add(&__c, clair_literal_3); nv_call_add(&__c, nv_int((long long)texte.len)); NvVal __r = nv_dispatch_call("ecris", __c.args, __c.argc, __c.kw); nv_call_free(&__c); __r; });
    clair_string_release(&texte);
    clair_string_release(&base);
    return 0;
}
