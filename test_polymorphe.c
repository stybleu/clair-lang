...
static NvVal clair_fn_addition(NvVal *__args, int __argc, NvDict *__kwargs) {
    NvVal a = nv_arg(__args, __argc, __kwargs, 0, "a");
    NvVal b = nv_arg(__args, __argc, __kwargs, 1, "b");
    return nv_add(a, b);
    return nv_none();
}
...
static NvVal nv_dispatch_call(const char *name,NvVal *args,int argc,NvDict *kw){
    ...
    if(strcmp(name,"addition")==0) return clair_fn_addition(args,argc,kw);
...
int main(void){
long long resultat_entier = (10LL + 20LL);
double resultat_decimal = (1.5 + 2.75);
double resultat_mixte = (10LL + 2.5);
(void)({ NvCall __c = nv_call_new(); nv_call_add(&__c, nv_str("Entier :")); nv_call_add(&__c, nv_int(resultat_entier)); NvVal __r = nv_dispatch_call("ecris", __c.args, __c.argc, __c.kw); nv_call_free(&__c); __r; });
(void)({ NvCall __c = nv_call_new(); nv_call_add(&__c, nv_str("Decimal :")); nv_call_add(&__c, nv_float(resultat_decimal)); NvVal __r = nv_dispatch_call("ecris", __c.args, __c.argc, __c.kw); nv_call_free(&__c); __r; });
(void)({ NvCall __c = nv_call_new(); nv_call_add(&__c, nv_str("Mixte :")); nv_call_add(&__c, nv_float(resultat_mixte)); NvVal __r = nv_dispatch_call("ecris", __c.args, __c.argc, __c.kw); nv_call_free(&__c); __r; });
    return 0;
}
