...
(void)({ NvCall __c = nv_call_new(); nv_call_add(&__c, nv_str("Complet :")); nv_call_add(&__c, clair_string_box(complet)); NvVal __r = nv_dispatch_call("ecris", __c.args, __c.argc, __c.kw); nv_call_free(&__c); __r; });
(void)({ NvCall __c = nv_call_new(); nv_call_add(&__c, nv_str("Longueur :")); nv_call_add(&__c, nv_int((long long)complet.len)); NvVal __r = nv_dispatch_call("ecris", __c.args, __c.argc, __c.kw); nv_call_free(&__c); __r; });
...
(void)({ NvCall __c = nv_call_new(); nv_call_add(&__c, nv_str("Premier caractere :")); nv_call_add(&__c, clair_string_box(clair_string_char(prenom, (long long)(0LL)))); NvVal __r = nv_dispatch_call("ecris", __c.args, __c.argc, __c.kw); nv_call_free(&__c); __r; });
    return 0;
}
