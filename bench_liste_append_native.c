...
int main(void){
ClairIntList nombres = clair_int_list_make((long long[]){0LL}, 1LL);
long long clair_range_index_1 = 0LL;
long long clair_range_fin_1 = 1000000LL;
...
(void)({ NvCall __c = nv_call_new(); nv_call_add(&__c, nv_str("Dernier :")); nv_call_add(&__c, nv_int(clair_int_list_get(&nombres, (long long)(1000000LL)))); NvVal __r = nv_dispatch_call("ecris", __c.args, __c.argc, __c.kw); nv_call_free(&__c); __r; });
    return 0;
}
