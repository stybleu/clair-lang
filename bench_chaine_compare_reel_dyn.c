...
int main(void){
NvVal gauche1 = nv_str("Al");
NvVal droite1 = nv_str("ice");
NvVal nom = nv_add(gauche1, droite1);
NvVal gauche2 = nv_str("A");
NvVal droite2 = nv_str("lice");
NvVal cible = nv_add(gauche2, droite2);
NvVal compteur = nv_int(0LL);
NvVal clair_range_index_1 = nv_int(0LL);
NvVal clair_range_fin_1 = nv_int(50000000LL);
NvVal clair_range_pas_1 = nv_int(1LL);
while (nv_truth(nv_lt(clair_range_index_1, clair_range_fin_1))) {
    NvVal i = clair_range_index_1;
    if (nv_truth(nv_eq(nom, cible))) {
        compteur = nv_add(compteur, nv_int(1LL));
    }
    clair_range_index_1 = nv_add(clair_range_index_1, clair_range_pas_1);
}
(void)({ NvCall __c = nv_call_new(); nv_call_add(&__c, compteur); NvVal __r = nv_dispatch_call("ecris", __c.args, __c.argc, __c.kw); nv_call_free(&__c); __r; });
    return 0;
}
