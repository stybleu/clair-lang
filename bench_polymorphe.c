...
int main(void){
long long entier_resultat = 0LL;
double decimal_resultat = 0.0;
double mixte_resultat = 0.0;
long long clair_range_index_1 = 0LL;
long long clair_range_fin_1 = 10000000LL;
long long clair_range_pas_1 = 1LL;
while (((clair_range_index_1 < clair_range_fin_1))) {
    long long i = clair_range_index_1;
    entier_resultat = (entier_resultat + 1LL);
    decimal_resultat = (decimal_resultat + 0.000001);
    mixte_resultat = (i + 0.5);
    clair_range_index_1 = (clair_range_index_1 + clair_range_pas_1);
}
(void)({ NvCall __c = nv_call_new(); nv_call_add(&__c, nv_int(entier_resultat)); NvVal __r = nv_dispatch_call("ecris", __c.args, __c.argc, __c.kw); nv_call_free(&__c); __r; });
(void)({ NvCall __c = nv_call_new(); nv_call_add(&__c, nv_float(decimal_resultat)); NvVal __r = nv_dispatch_call("ecris", __c.args, __c.argc, __c.kw); nv_call_free(&__c); __r; });
(void)({ NvCall __c = nv_call_new(); nv_call_add(&__c, nv_float(mixte_resultat)); NvVal __r = nv_dispatch_call("ecris", __c.args, __c.argc, __c.kw); nv_call_free(&__c); __r; });
    return 0;
}
