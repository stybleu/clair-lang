...
int main(void){
NvVal valeurs = ({ NvVal __l = nv_list_new(); nv_list_append(__l, nv_float(1.5)); nv_list_append(__l, nv_float(2.5)); nv_list_append(__l, nv_float(3.5)); nv_list_append(__l, nv_float(4.5)); nv_list_append(__l, nv_float(5.5)); __l; });
NvVal somme = nv_float(0.0);
NvVal clair_range_index_1 = nv_int(0LL);
NvVal clair_range_fin_1 = nv_int(100000000LL);
NvVal clair_range_pas_1 = nv_int(1LL);
while (nv_truth(nv_lt(clair_range_index_1, clair_range_fin_1))) {
    NvVal i = clair_range_index_1;
    somme = nv_add(somme, nv_get_index(valeurs, nv_mod(i, nv_int(5LL))));
    clair_range_index_1 = nv_add(clair_range_index_1, clair_range_pas_1);
}
(void)({ NvCall __c = nv_call_new(); nv_call_add(&__c, somme); NvVal __r = nv_dispatch_call("ecris", __c.args, __c.argc, __c.kw); nv_call_free(&__c); __r; });
    return 0;
}
