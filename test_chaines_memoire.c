...
static NvVal clair_literal_2;
static NvVal clair_literal_3;
static NvVal clair_literal_4;
static NvVal clair_literal_5;
...
int main(void){
    ...
    clair_literal_2 = nv_str("Complet :");
    clair_literal_3 = nv_str("Longueur :");
    clair_literal_4 = nv_str("Comparaison OK");
    clair_literal_5 = nv_str("Premier caractere :");
ClairString prenom = clair_string_literal("Alice");
ClairString nom = clair_string_literal("Martin");
ClairString complet = clair_string_concat_many((ClairString[]){prenom, clair_string_literal(" "), nom}, 3LL);
(void)({ NvCall __c = nv_call_new(); nv_call_add(&__c, clair_literal_2); nv_call_add(&__c, clair_string_box(complet)); NvVal __r = nv_dispatch_call("ecris", __c.args, __c.argc, __c.kw); nv_call_free(&__c); __r; });
(void)({ NvCall __c = nv_call_new(); nv_call_add(&__c, clair_literal_3); nv_call_add(&__c, nv_int((long long)complet.len)); NvVal __r = nv_dispatch_call("ecris", __c.args, __c.argc, __c.kw); nv_call_free(&__c); __r; });
if (nv_truth(nv_bool(clair_string_eq(prenom, clair_string_literal("Alice"))))) {
    (void)({ NvCall __c = nv_call_new(); nv_call_add(&__c, clair_literal_4); NvVal __r = nv_dispatch_call("ecris", __c.args, __c.argc, __c.kw); nv_call_free(&__c); __r; });
}
(void)({ NvCall __c = nv_call_new(); nv_call_add(&__c, clair_literal_5); nv_call_add(&__c, clair_string_box(clair_string_char(prenom, (long long)(0LL)))); NvVal __r = nv_dispatch_call("ecris", __c.args, __c.argc, __c.kw); nv_call_free(&__c); __r; });
    return 0;
}
