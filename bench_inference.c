...
static NvVal clair_fn_calcule(NvVal *__args, int __argc, NvDict *__kwargs) {
    NvVal a = nv_arg(__args, __argc, __kwargs, 0, "a");
    NvVal b = nv_arg(__args, __argc, __kwargs, 1, "b");
    return nv_mod((nv_add(nv_mul(a, nv_int(3LL)), b)), nv_int(1000000007LL));
    return nv_none();
}
...
static NvVal nv_dispatch_call(const char *name,NvVal *args,int argc,NvDict *kw){
    ...
    if(strcmp(name,"calcule")==0) return clair_fn_calcule(args,argc,kw);
...
while (((clair_range_index_1 < clair_range_fin_1))) {
    ...
    x = ((long long)(((x * 3LL) + i)) % (long long)(1000000007LL));
    clair_range_index_1 = (clair_range_index_1 + clair_range_pas_1);
}
(void)({ NvCall __c = nv_call_new(); nv_call_add(&__c, nv_int(x)); NvVal __r = nv_dispatch_call("ecris", __c.args, __c.argc, __c.kw); nv_call_free(&__c); __r; });
    return 0;
}
