...
int main(void){
NvVal nombres = ({ NvVal __l = nv_list_new(); nv_list_append(__l, nv_int(0LL)); __l; });
NvVal clair_range_index_1 = nv_int(0LL);
NvVal clair_range_fin_1 = nv_int(1000000LL);
NvVal clair_range_pas_1 = nv_int(1LL);
while (nv_truth(nv_lt(clair_range_index_1, clair_range_fin_1))) {
    NvVal i = clair_range_index_1;
    ...
    clair_range_index_1 = nv_add(clair_range_index_1, clair_range_pas_1);
}
...
(void)({ NvCall __c = nv_call_new(); nv_call_add(&__c, nv_str("Dernier :")); nv_call_add(&__c, nv_get_index(nombres, nv_int(1000000LL))); NvVal __r = nv_dispatch_call("ecris", __c.args, __c.argc, __c.kw); nv_call_free(&__c); __r; });
    return 0;
}
